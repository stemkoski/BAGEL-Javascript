...
class TitleScreen extends BAGEL.Screen 
{
	initialize()
	{
		...
		background.setTexture(
    	  new BAGEL.Texture("assets/space-rocks/space.png") );
		...
		let rockTextureArray = [
			new BAGEL.Texture("assets/space-rocks/rock1.png"),
			new BAGEL.Texture("assets/space-rocks/rock2.png"),
			new BAGEL.Texture("assets/space-rocks/rock3.png"),
			new BAGEL.Texture("assets/space-rocks/rock4.png")
		];
		...
		title.setTexture(
    	  new BAGEL.Texture("assets/space-rocks/space-rocks-title.png") );
		...
		// set text this.label.setText("press [P]");
		...
		this.highScoreLabel.fontSize = 20; // default 36
    ...
    resume()
    {
		...
		this.game.addScreen("level", new LevelScreen(this.game) );
    }
	...
	update()
	{
		if ( this.game.input.gamepad.active )
		{
			...
			if ( this.game.input.gamepad.buttonPressed("Y") )
			    this.game.setScreen("instructions");
		}		
		else // keyboard controls
		{
			this.label.setText("press [P]");
		    if ( this.game.input.keyPressed("P") )
			    this.game.setScreen("instructions");
		}
...
class InstructionsScreen extends BAGEL.Screen 
{
	initialize()
	{
		let background = new BAGEL.Sprite();
		...
		this.fire.setTexture(
		  new BAGEL.Texture("assets/space-rocks/fire.png") );
		...
		this.spaceship.setTexture(
		  new BAGEL.Texture("assets/space-rocks/spaceship.png") );
		...
		this.laserTexture = new BAGEL.Texture("assets/space-rocks/laser.png");
		this.createGroup("laser");

		this.warpAnimation = new BAGEL.Animation(
            "assets/space-rocks/warp.png", 4, 8, 0.03, true );
	...
	update(deltaTime)
	{
		if ( this.game.input.gamepad.active )
		{
			this.label1.setText("gamepad controls \n"
			         + "(try them!)");
			// unicode: circled A B X Y 24B6 24B7 24CD 24CE -- too hard to read
			this.label2.setText("joystick \u{1F850}: turn left \n"
			         + "joystick \u{1F852}: turn right \n"
		...
		{
			this.label1.setText("keyboard controls \n"
			         + "(try them!)");
			this.label2.setText("[\u{1F850}]: turn left \n"
			         + "[\u{1F852}]: turn right \n"
			         + "[\u{1F851}]: accelerate forward \n"
			         + "[Space]: shoot laser \n"
			         + "[X]: warp (random) \n"
			         + "[P]: pause (during game)" );
			this.label3.setText("press [P] to start");
		}
		...
		{	
			...
			let dx = this.game.input.gamepad.getAxisValue("Axis1X");
			this.rotateSpaceship(dx * angle);

			if ( this.game.input.gamepad.buttonPressing("A") )
				this.accelerateSpaceship(1.00);
			else
				this.accelerateSpaceship(0.00);

			// shoot laser; at most 4 on screen at any time
			if ( this.game.input.gamepad.buttonPressed("X") && this.getGroupSpriteCount("laser") < 4 )
				this.shootLaser();

			// warp to new random location on screen (useful when unable to escape incoming rock)
			if ( this.game.input.gamepad.buttonPressed("B") )
		...
		else // keyboard controls
		{
			...
			// shoot laser; at most 4 on screen at any time
			if ( this.game.input.keyPressed(" ") && this.getGroupSpriteCount("laser") < 4 )
				this.shootLaser();
...
class LevelScreen extends BAGEL.Screen 
{
	initialize()
	{
		...
		this.spaceship.setTexture(
 		...
 		this.shields.setTexture(
		  new BAGEL.Texture("assets/space-rocks/shields.png") );
		...
		this.sparkAnimation = new BAGEL.Animation(
            "assets/space-rocks/spark.png", 4, 4, 0.03, true );
		...
		this.levelCompleteLabel.setTexture(
		  new BAGEL.Texture("assets/space-rocks/level-complete.png") );
		...
		this.gameOverLabel.setTexture(
		  new BAGEL.Texture("assets/space-rocks/game-over.png") );
		...
		this.pauseOverlay.setTexture(
		  new BAGEL.Texture("assets/space-rocks/black.png") );
		...
		// set text later, depending on active control system
	...
	update(deltaTime)
	{
		...
		if ( this.gameOver )
		{
			...
			if ( this.game.input.keyPressed("X") ||
			   ( this.game.input.gamepad.active && this.game.input.gamepad.buttonPressed("X") ) )
		...
		if ( this.game.input.keyPressed("P") ||
		   ( this.game.input.gamepad.active && this.game.input.gamepad.buttonPressed("Y") ) )
 
		{
			if ( this.game.input.gamepad.active )
				this.pauseLabel2.setText("press (Y) to resume, or (X) to quit");
			else // keyboard controls	
				this.pauseLabel2.setText("press [P] to resume, or [X] to quit");
